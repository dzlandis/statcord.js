...
class ShardingClient extends EventEmitter {
  ...
  constructor(options) {
    ...
    try {
      ...
      this.discord_hybrid_sharding = require('discord-hybrid-sharding');
    } catch (e) {
      throw new Error(
        'statcord.js needs discord.js and discord_hybrid_sharding to function'
      );
    }
    ...
    if (!key.startsWith('statcord.com-'))
      throw new Error(
        '"key" is not prefixed by "statcord.com-", please follow the key format'
      );
    ...
    if (typeof autopost !== 'boolean')
      throw new TypeError('"autopost" is not of type boolean');
    // Post arg error checking
    if (postCpuStatistics == null || postCpuStatistics == undefined)
      postCpuStatistics = true;
    if (typeof postCpuStatistics !== 'boolean')
      throw new TypeError('"postCpuStatistics" is not of type boolean');
    if (postMemStatistics == null || postMemStatistics == undefined)
      postMemStatistics = true;
    if (typeof postMemStatistics !== 'boolean')
      throw new TypeError('"postMemStatistics" is not of type boolean');
    if (postNetworkStatistics == null || postNetworkStatistics == undefined)
      postNetworkStatistics = true;
    if (typeof postNetworkStatistics !== 'boolean')
      throw new TypeError('"postNetworkStatistics" is not of type boolean');
    ...
    // Check if all clusters have been spawned
    this.manager.on('clusterCreate', (cluster) => {
      // Get current shard
      let currCluster = this.manager.clusters.get(cluster.id);

      // If this is the last cluster, wait until it is ready
      if (cluster.id + 1 == this.manager.totalClusters && autopost) {
        // When ready start auto post
        this.debugLog('Listening for final cluster "ready" event');
        currCluster.once('ready', () => {
          setTimeout(async () => {
      ...
      currCluster.on('message', async (message) => {
        ...
        if (args[0] == 'sscpc') {
          // PostCommand message
          this.debugLog('Received command post from cluster');
          this.postCommand(args[1], args[2]);
        } else if (args[0] == 'sscp') {
          // Post message
          this.debugLog('Received full post request from cluster');
          this.post();
        }
      });
    });
  ...
  async post() {
    ...
    let user_count = require('discord.js').version.startsWith('13')
      ? await getUserCountV13(this.manager)
      : await getUserCountV12(this.manager);

    // Get and sort popular commands
    let popular = [];

    let sortedPopular = this.popularCommands
    ...
    try {
      ...
      this.emit(
        'post',
        'Unable to connect to the Statcord server. Going to automatically try again in 60 seconds, if this problem persists, please visit status.statcord.com'
      );
    ...
    if (response.status >= 500) {
      ...
      this.emit(
        'post',
        new Error(`Statcord server error, statuscode: ${response.status}`)
      );
      return;
    }
    ...
    try {
      ...
      this.emit(
        'post',
        new Error(`Statcord server error, invalid json response`)
      );
      return;
    }
  ...
  async postCommand(command_name, author_id) {
    ...
    if (!command_name)
      throw new Error('"command_name" is missing or undefined');
    if (typeof command_name !== 'string')
      throw new TypeError('"command_name" is not typeof string');
    // Author id error checking
    if (!author_id) throw new Error('"author_id" is missing or undefined');
    if (typeof author_id !== 'string')
      throw new TypeError('"author_id" is not typeof string');
    ...
    if (!this.popularCommands.some((command) => command.name == command_name)) {
      ...
      let commandIndex = this.popularCommands.findIndex(
  ...
  async registerCustomFieldHandler(customFieldNumber, handler) {
    ...
    if (this.customFields.get(customFieldNumber))
      throw new Error('Handler already exists');

    // Testing
    if (typeof handler !== 'function')
      throw new Error('Handler is not a function');
    if (typeof (await handler(this.manager)) !== 'string')
      throw new Error("Handler doesn't return strings");
  ...
  debugLog(info, type = '') {
    ...
    if (this.debug_outfile) {
      fs.appendFile(
        require('path').join(process.cwd(), this.debug_outfile),
        out + '\n',
        (err) => {
...
async function getUserCountV13(manager) {
  const memberNum = await manager.broadcastEval((client) =>
