...
module.exports.postCommand = (command_name, author_id, client) => {
  ...
  if (!(client instanceof Discord.Client))
    throw new Error('"client" is not a discord.js client');
  if (!client.cluster)
    throw new Error('This function is only for clustered clients');

  // Send message with args to Statcord Sharding/Cluster Client
  client.cluster.send(`sscpc|=-ssc-=|${command_name}|=-ssc-=|${author_id}`);
};
...
module.exports.post = (client) => {
  ...
  if (!(client instanceof Discord.Client))
    throw new Error('"client" is not a discord.js client');
  if (!client.cluster)
    throw new Error('This function is only for sharded clients');

  // Send message with args to Statcord Sharding/Cluster Client
  client.cluster.send(`sscp|=-ssc-=|`);
};
